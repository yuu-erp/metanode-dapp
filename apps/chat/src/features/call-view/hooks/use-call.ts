...
const eventLog = container.eventLogContainer
...
export function useCall() {
  ...
  const setupCall = async () => {
    const { isCaller, address, callee, isMeet, hiddenAddress } = callContext
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      const { isDone } = useCallStore.getState()
      if (!isDone) eventBus.emit('call.end', null)
    }, 60_000)
  ...
  useEffect(() => {
    const cb = () => eventBus.emit('call.end', null)
